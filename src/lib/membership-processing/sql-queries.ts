...
export function newMemberSQLQuery(member: Member, password: string): string {
  return `INSERT INTO WYCDatabase
  (WYCNumber, Last, First, StreetAddress, City, State, ZipCode, Phone1, Phone2, Email, Category, ExpireQtr, out_to_sea, password)
VALUES
  (${member.wycNumber},
  '${esc(member.last)}',
  '${esc(member.first)}',
